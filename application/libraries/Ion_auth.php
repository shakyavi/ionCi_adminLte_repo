...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$email_config = $this->config->item('email_config', 'ion_auth');
                                     //echo "<br>email_config<br><br>".implode(",<br>",$email_config);
      ...
      //echo "<br> user = ".$userC;
			...
			{   echo "<br> inside if user<br>";
				...
				{               echo "<br> use_ci_email <br>";
					...
					$this->email->from('bjack0470@gmail.com','ion_Auth Check Chek');
                                        $this->email->to($user->email);                                                        
                                        //$this->email->to('anzel.shrestha@gmail.com');
					$this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
					$this->email->message($message);
                                                                                //debugging
                                                                                    //$da = $this->email;
                                                                                   // echo "<br> email : ".implode(",<br>", $da);
                                                                                    //echo "<br> message = ".$message;
					if ($this->email->send())
					{               echo  "<br> if this-email-send <br>";
						$this->set_message('forgot_password_successful');
						return TRUE;
					}
					else
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids);
                                    //echo "<br>LIBRARY ION_AUTH<br>"+ $id;
		if (!$email_activation)
		{     //echo "<br>LIBRARY ION_AUTH<br>";
			if ($id !== FALSE)
			{
