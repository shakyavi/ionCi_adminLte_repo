 ...
 * @return string
 */
function cke_create_instance($id = 'content', $data = array()) {
	    		...
	    		foreach($data['config'] as $k=>$v) {
	    			...
	    			if (is_array($v)) {
	    				...
	    				$return .= $k . " : '" . $v . "'";
                                                                            ...
                                                                            $varr =(array)array_keys($data['config']);
                                                                            if($k !== end($varr)) {
						$return .= ",";
					}
 ...
 * @return string
 */
function display_ckeditor($id, $toolbar = 'Full')
{
	$CI =& get_instance();
        
        $CI->load->config('ckeditor_config', TRUE);
	
        $data = $CI->config->item($toolbar,'ckeditor_config');
		// Initialization
		$return = cke_initialize($data);

		// Creating a Ckeditor instance
		$return .= cke_create_instance($id, $data);


		// Adding styles values
		if(isset($data['styles'])) {

			$return .= "<script type=\"text/javascript\">CKEDITOR.addStylesSet( 'my_styles_" . $id . "', [";
			...
			$return .= "CKEDITOR.instances['" . $id . "'].config.stylesCombo_stylesSet = 'my_styles_" . $id . "';
			</script>";		
		}   
